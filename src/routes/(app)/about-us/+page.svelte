<script>
	import AboutUsImage from "$lib/assets/images/School.jpg"
	import { fade } from 'svelte/transition';
</script>

<section>
	<div transition:fade class="relative h-96 bg-center bg-cover w-full" style="background-image: url({AboutUsImage})">
		<div class="absolute inset-o h-full bg-black/50 w-full items-center
		flex content-center justify-center">
			<h1 class="text-center font-extrabold text-6xl capitalize text-white">about us</h1>
		</div>
	</div>

	<div class="px-5 lg:px-10 py-14 space-y-5">

		<div>
			<div>
				<h1 class="capitalize text-3xl font-bold">about aglit</h1>
			</div>
			<div class="pt-5">
				<p class="leading-7 text-lg">
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
				</p>
			</div>
		</div>

		<div>
			<div>
				<h1 class="capitalize text-3xl font-bold">about aglit</h1>
			</div>
			<div class="pt-5">
				<p class="leading-7 text-lg">
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia et laborum adipisci,
					repellat quam delectus amet tempora nemo eum debitis non error expedita aut. Expedita,
					beatae. Labore ab vero quibusdam.
				</p>
			</div>
		</div>

	</div>

</section>